...
def getSoftConstraintScore(population, tOff):
    Register = np.tile(tOff, (population.shape[0], DAY_NO)).reshape(
        -1, *population.shape[1:]
    )[:, :, :1]
    CheckRef = np.take_along_axis(population, Register, axis=2)

    # Constraint3
    fitness3 = np.zeros_like(CheckRef)
    criteria_3 = CheckRef == 0
    fitness3[criteria_3] = 4
    score3 = fitness3.sum(axis=1)
    return score3


def getTotalGoodness(population, tOff):
    score2 = getHardConstraintScore(population)
    score3 = getSoftConstraintScore(population, tOff)
...
def saveToExcel(sheets):
    with pd.ExcelWriter("GAOff.xlsx") as writer:
...
if __name__ == "__main__":
    ...
    df = pd.read_excel("GAOff.xlsx", sheet_name=None)
    tOff = df["Off"].iloc[:offRotation].set_index("Em").to_numpy() - 1
    ...
    finalScore, _, _ = getTotalGoodness(population, tOff)
    ...
    for iteration in range(200):  # (run 3 times => interation < 3)
        ...
        # find worst schedule
        finalScore, _, _ = getTotalGoodness(mutationOffspring, tOff)
        ...
        finalScore, score2, score3 = getTotalGoodness(newGeneration, tOff)
